...
plot.it(t, i) # Current vs t
plot.iE(E1, i[0:np.size(E1)]) # CVs
plot.it(t2, i[np.size(E1):np.size(E1) + np.size(E2)]) # First step
plot.it(t3, i[np.size(E1) + np.size(E2):]) # Second step
